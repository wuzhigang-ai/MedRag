...
7. 列举文献/数据时必须逐条核对、不得遗漏:
   - 表格行数必须等于实际文献/条目数。例如 list_docs 返回3篇文献，表格必须列出3行，不得漏写任何一篇。
   - 知识库覆盖范围表格中，column数量必须与 search_rag 或 list_docs 返回的条目数完全一致。
   - 输出前自检: 逐行核对表格中的文献名与工具返回的文献名是否一一对应。
   - 反例: list_docs返回[A,B,C]三篇，表格只列了A和B → 这是错误的。必须列出A、B、C全部三篇。
"""
...
输出JSON: {{"chart_type":"{chart_type}","key_findings":"","description":"","clinical_significance":""}}""")
    ...
    def run(self, query: str, max_steps: int = 20,
        ...
        return {
            "answer": (
                "很高兴你愿意和我交流！😊 我是专为医疗文献检索和循证医学分析设计的 AI 助手，"
                "目前在心血管疾病、遗传代谢病和肝移植等领域有丰富的知识储备。\n\n"
                "你可以这样向我提问：\n"
                "- LVAD 与 IABP 在治疗心源性休克的疗效对比？\n"
                "- TBAD 的诊断标准和治疗策略？\n"
                "- 丙酸血症的遗传模式和临床表现？\n"
                "- 肝移植术后免疫抑制方案的选择？\n\n"
                "我会从已入库的文献中为你检索最可靠的循证医学证据。期待和你一起探索医学知识！💙"
            ),
            "reasoning_trace": reasoning_trace,
            "steps": max_steps,
            "model": self.model,
